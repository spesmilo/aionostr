...
docker build \
    $DOCKER_BUILD_FLAGS \
    -t electrum-aionostr-builder-img \
    "$CONTRIB_SDIST"

# maybe do fresh clone
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    echo "ELECBUILD_COMMIT=$ELECBUILD_COMMIT. doing fresh clone and git checkout."
    FRESH_CLONE="/tmp/electrum_aionostr_build/fresh_clone/electrum_aionostr"
...
docker run -it \
    --name electrum-aionostr-builder-cont \
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/electrum_aionostr \
    --rm \
    --workdir /opt/electrum_aionostr/contrib/sdist \
    electrum-aionostr-builder-img \
    ./make_sdist.sh
